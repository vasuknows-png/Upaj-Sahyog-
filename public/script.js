...
// Product photos are selected from the crop NAME, never from a database ID.
// Database IDs change between installations and previously caused mismatched photos.
const WIKI = (file) => 'https:' + '//commons.wikimedia.org/wiki/Special:FilePath/' + encodeURIComponent(file) + '?width=640';
const CROP_IMAGE_RULES = [
  {keys:['pomegranate','अनार'], src:'assets/crops/pomegranate.png'},
  {keys:['grape','अंगूर'], src:'assets/crops/grapes.png'},
  {keys:['basmati','rice','चावल'], src:'assets/crops/rice.png'},
  {keys:['tomato','टमाटर'], src:'assets/crops/tomato.png'},
  {keys:['onion','प्याज़','प्याज'], src:'assets/crops/onion.png'},
  {keys:['potato','आलू'], src:'assets/crops/potato.png'},
  {keys:['wheat','गेहूं','गेहूँ'], src:'assets/crops/wheat.png'},
  {keys:['maize','corn','मक्का'], src:'assets/crops/maize.png'},
  {keys:['mango','आम'], src:'assets/crops/mango.png'},
  {keys:['orange','संतरा'], src:'assets/crops/orange.png'},
  {keys:['toor','arhar','अरहर'], src:'assets/crops/toor.png'},
  {keys:['chana','चना'], src:'assets/crops/chana.png'},
  {keys:['lentil','masoor','मसूर'], file:'Red lentils.jpg'},
  {keys:['spinach','पालक'], file:'Spinach leaves.jpg'},
  {keys:['carrot','गाजर'], file:'Carrots.JPG'},
  {keys:['chilli','chili','मिर्च'], file:'Chili pepper.jpg'},
  {keys:['broccoli'], file:'Broccoli.jpg'},
  {keys:['banana','केला'], file:'Bananas.jpg'},
  {keys:['apple','सेब'], file:'Red Apple.jpg'},
];
const EMOJI_IMAGES = {
  '🌾': WIKI('Wheat grain.jpg'), '🌽': WIKI('Corn on the cob.jpg'),
  '🍅': WIKI('Tomatoes.jpg'), '🧅': WIKI('Red onions.jpg'),
  '🥔': WIKI('Potatoes.jpg'), '🥬': WIKI('Spinach leaves.jpg'),
  '🫘': WIKI('Chana dal.jpg'), '🥭': WIKI('Large mango.jpg'),
  '🍊': WIKI('Orange fruit.jpeg'), '🍇': WIKI('Table grapes on white.jpg'),
  '🍎': WIKI('Red Apple.jpg'), '🍌': WIKI('Bananas.jpg'),
  '🥕': WIKI('Carrots.JPG'), '🌶️': WIKI('Chili pepper.jpg'),
  '🥦': WIKI('Broccoli.jpg')
};
const CATEGORY_FALLBACK = {
  grain:'🌾', veg:'🥬', vegetable:'🥬', pulse:'🫘', fruit:'🍊'
};
function productImageUrl(productId, emoji, nameEn='', nameHi='', category=''){
  const cropName = `${nameEn || ''} ${nameHi || ''}`.toLocaleLowerCase();
  const rule = CROP_IMAGE_RULES.find(r => r.keys.some(k => cropName.includes(k.toLocaleLowerCase())));
  if(rule) return rule.src || WIKI(rule.file);
  return EMOJI_IMAGES[emoji] || EMOJI_IMAGES[CATEGORY_FALLBACK[category]] || null;
}
// A failed remote photo falls back to the correct crop emoji, never another product.
function mediaHtml(emoji, productId, nameEn='', nameHi='', category=''){
  const url = productImageUrl(productId, emoji, nameEn, nameHi, category);
  const label = String(nameEn || nameHi || 'Farm produce').replace(/"/g,'&quot;');
  if(url){
    return `<img class="prod-img" src="${url}" alt="${label}" loading="lazy" onerror="this.replaceWith(Object.assign(document.createElement('span'),{className:'prod-img-fallback',textContent:'${emoji || CATEGORY_FALLBACK[category] || '🌾'}'}))">`;
  }
  return `<span class="prod-img-fallback" role="img" aria-label="${label}">${emoji || CATEGORY_FALLBACK[category] || '🌾'}</span>`;
}
  ...
  grid.innerHTML = list.map(p=>{
    ...
    <div class="card">
      <div class="card-media">
        ...
        ${mediaHtml(p.emoji, p.id, p.nameEn, p.nameHi, p.cat)}
      </div>
      <div class="card-body">
...
function renderCart(){
  ...
  document.getElementById('cartItemsWrap').innerHTML = ids.map(id=>{
    ...
    return `
    <div class="cart-item">
      <div class="ci-media">${mediaHtml(p.emoji, p.id, p.nameEn, p.nameHi, p.cat)}</div>
      <div class="ci-info">
...
function renderFarmer(){
  ...
  document.getElementById('farmerListings').innerHTML = state.myListings.map(l=>`
    <div class="farm-row">
      <div class="fi">
        <div class="emoji">${mediaHtml(l.emoji, l.productId, l.nameEn, l.nameHi, l.cat)}</div>
        <div>
